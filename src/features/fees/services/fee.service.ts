...
export class FeeService {
   ...
   */
  static async getTeacherStudentStatuses(teacherUserId: string) {
    const teacher = await prisma.teacher.findFirst({
      where: { userId: teacherUserId, deletedAt: null },
      select: { id: true },
    });
    if (!teacher) return [] as Array<any>;

    const assignments = await prisma.teacherAssignment.findMany({
      where: { teacherId: teacher.id, deletedAt: null },
      select: { batchId: true, courseId: true },
    });
    if (assignments.length === 0) return [] as Array<any>;

    const byBatch = new Map<string, Set<string>>();
    for (const a of assignments) {
      const set = byBatch.get(a.batchId) || new Set<string>();
      set.add(a.courseId);
    ...
    return enrollments
      ...
      .map((e) => {
        ...
        const paid = inv
          ? inv.payments?.reduce((sum: number, p: any) => sum + Number(p.amount || 0), 0) || 0
          : 0;
        ...
        if (inv) {
          if (inv.status === 'PAID') status = 'Cleared';
          else if (remaining === 0) status = 'Cleared';
          else if (inv.dueDate && now > inv.dueDate) status = 'Overdue';
        ...
        return {
          ...
          feeType: inv?.feeStructure ? (inv as any).feeStructure?.name : undefined,
        };
      });
  }
}
