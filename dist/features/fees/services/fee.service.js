...
class FeeService {
    ...
    static async createFeeStructure(data, userId) {
        const feeStructure = await client_1.prisma.feeStructure.create({
            data: {
     ...
     */
    static async updateFeeStructure(feeStructureId, data, userId) {
        const feeStructure = await client_1.prisma.feeStructure.update({
     ...
     */
    static async listFeeStructures(query) {
        ...
        const [structures, total] = await Promise.all([
            client_1.prisma.feeStructure.findMany({
                where: whereClause,
                skip,
                take: limit,
                include: { batch: true, course: true },
            }),
            client_1.prisma.feeStructure.count({ where: whereClause }),
        ]);
        return {
            structures,
     ...
     */
    static async calculateFee(feeStructureId, attendedClasses = 0) {
        const feeStructure = await client_1.prisma.feeStructure.findUnique({
            where: { id: feeStructureId },
        });
        if (!feeStructure)
     ...
     */
    static async checkAndCreatePaymentDue(studentId, courseId, batchId, userId) {
        const attendanceRecords = await client_1.prisma.attendanceRecord.findMany({
            where: {
                studentId,
                courseId,
                deletedAt: null,
        ...
        const feeStructure = await client_1.prisma.feeStructure.findFirst({
            where: {
                batchId,
                courseId,
                isActive: true,
        ...
        if (feeStructure.amountPerClass.toNumber() > 0 && feeStructure.classesRequired > 0) {
            ...
            const totalClasses = await client_1.prisma.attendanceRecord.findMany({
            ...
            if (attendancePercentage >= (feeStructure.classesRequired > 0 ? 100 : 0)) {
                ...
                const existingInvoice = await client_1.prisma.invoice.findFirst({
                    where: {
                        studentId,
                ...
                if (!existingInvoice) {
                    ...
                    await client_1.prisma.notification.create({
                        data: {
                            userId: (await client_1.prisma.student.findUnique({ where: { id: studentId }, select: { userId: true } }))?.userId || '',
                            type: 'FEE',
                            title: 'Payment Due',
     ...
     * Record a payment
     */
    static async recordPayment(data, userId) {
        ...
        const totalPaid = await client_1.prisma.payment.aggregate({
    ...
    static async getPaymentHistory(query) {
        ...
        const [invoices, total] = await Promise.all([
            ...
            client_1.prisma.invoice.count({ where: whereClause }),
        ]);
        return {
    ...
    static async getRevenueReport(query) {
        ...
        const whereClause = { status: 'PAID', deletedAt: null };
        if (batchId || courseId) {
        ...
        return {
            summary: {
                totalRevenue: invoices.reduce((sum, inv) => sum + inv.payments.reduce((s, p) => s + Number(p.amount), 0), 0),
                totalInvoices: invoices.length,
                avgInvoiceValue: invoices.length > 0 ? invoices.reduce((sum, inv) => sum + inv.payments.reduce((s, p) => s + Number(p.amount), 0), 0) / invoices.length : 0,
            },
    ...
    static async getPendingPayments() {
        const pending = await client_1.prisma.invoice.findMany({
