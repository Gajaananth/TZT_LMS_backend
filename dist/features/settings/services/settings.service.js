...
const KNOWN_GROUPS = ['general', 'appearance', 'integrations'];
...
const seedMissingWithDefaults = async (userId) => {
    const allDefaults = {
        ...DEFAULT_GENERAL_KEYS,
        ...DEFAULT_APPEARANCE_KEYS,
        ...DEFAULT_INTEGRATION_KEYS,
    };
    const existing = await client_1.prisma.systemSetting.findMany({
    ...
    if (missing.length === 0)
        return;
    const now = new Date();
    await client_1.prisma.systemSetting.createMany({
...
class SettingsService {
    static async listSettings(group, userId) {
        await seedMissingWithDefaults(userId);
        const rows = await client_1.prisma.systemSetting.findMany({
    ...
    static async bulkUpdateSettings(records, userId) {
        ...
        for (const rec of records) {
            if (!rec || typeof rec.key !== 'string')
                continue;
            const existing = await client_1.prisma.systemSetting.findUnique({ where: { key: rec.key } });
            const value = rec.value ?? '';
            const wasMasked = /^●+$/.test(value);
            if (wasMasked)
                continue;
            if (existing) {
                await client_1.prisma.systemSetting.update({
                    where: { key: rec.key },
                    data: { value, updatedAt: now, updatedBy: userId },
                });
            }
            else {
                await client_1.prisma.systemSetting.create({
    ...
    static async assignUserRoles(targetUserId, roleIds, assignedByUserId) {
        const target = await client_1.prisma.user.findUnique({ where: { id: targetUserId } });
        if (!target)
            throw new Error('User not found');
        const existingSuperAdmin = await client_1.prisma.userRole.findFirst({
            where: { userId: targetUserId, role: { name: 'SuperAdmin' }, deletedAt: null },
            include: { role: { select: { id: true, name: true } } },
        });
        const callerIsSuperAdmin = await client_1.prisma.userRole.findFirst({
        ...
        const validRoles = await client_1.prisma.role.findMany({
