...
const normalizeType = (t) => {
    ...
    if (v === 'MSQ' || v === 'MULTIPLESELECT')
        return 'MULTIPLE_SELECT';
    if (v === 'TF' || v === 'TRUEFALSE')
        return 'TRUE_FALSE';
    if (v === 'FIB' || v === 'FILLINBLANK')
...
const ensureCategoryId = async (categoryId) => {
    if (typeof categoryId === 'string' && categoryId.length > 0)
        return categoryId;
    let existing = await client_1.prisma.questionCategory.findFirst({ where: { name: 'General' } });
    if (!existing) {
        existing = await client_1.prisma.questionCategory.create({ data: { name: 'General' } });
    }
    return existing.id;
};
...
const normalizeTrueFalseString = (v) => {
 ...
 */
const validateAndCoerceQuestionPayload = async (raw) => {
    ...
    let correctAnswer = raw.correctAnswer ?? null;
    ...
    switch (type) {
        ...
        case 'DROPDOWN': {
            ...
            if (Array.isArray(correctAnswer)) {
                if (correctAnswer.length === 0)
                    throw new Error('correctAnswer cannot be empty');
            ...
            if (typeof correctAnswer === 'string') {
                ...
                if (/^\d+$/.test(trimmed)) {
                    ...
                    if (options && idx >= options.length) {
                        throw new Error(`correctAnswer index ${idx} out of range (options.length=${options.length})`);
                    }
                    correctAnswer = idx;
                }
                ...
                if (!Number.isInteger(correctAnswer))
                    throw new Error('correctAnswer index must be integer');
                if (options && (correctAnswer < 0 || correctAnswer >= options.length)) {
                    throw new Error(`correctAnswer index ${correctAnswer} out of range (options.length=${options.length})`);
                }
            }
            else {
        ...
        case 'MULTIPLE_SELECT': {
            ...
            correctAnswer = correctAnswer.map((v) => {
                if (typeof v === 'string') {
                    ...
                    if (!Number.isInteger(v))
                        throw new Error('correctAnswer indices must be integers');
...
class QuestionService {
    static async listQuestions(page = 1, limit = 20, filter = {}) {
        ...
        const [items, total] = await Promise.all([
            client_1.prisma.question.findMany({
                where,
                skip,
                take: limit,
                orderBy: { createdAt: 'desc' },
            }),
            client_1.prisma.question.count({ where }),
        ]);
        return {
            data: items,
    ...
    static async createQuestion(data, userId) {
        ...
        return client_1.prisma.question.create({
            data: {
                ...validated,
                type: validated.type,
    ...
    static async updateQuestion(id, data, userId) {
        const existing = await client_1.prisma.question.findUnique({ where: { id } });
        if (!existing)
        ...
        return client_1.prisma.question.update({
            where: { id },
            data: {
                ...validated,
                type: validated.type,
