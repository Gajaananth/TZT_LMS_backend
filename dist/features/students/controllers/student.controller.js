...
async function canAccessStudentRecord(reqUser, studentRecordId) {
    if (!reqUser)
        return false;
    const roles = (reqUser.userRoles || []).map((ur) => ur.role?.name?.toLowerCase() || '');
...
class StudentController {
    ...
    static async createStudent(req, res, next) {
        try {
            ...
            if (supabaseError || !supabaseUser?.user) {
                return (0, api_response_1.sendError)(res, supabaseError?.message || 'Failed to create user in authentication system', 400);
            }
            // Get student role
            const studentRole = await client_1.prisma.role.findUnique({
                where: { name: 'Student' },
            });
            if (!studentRole) {
                return (0, api_response_1.sendError)(res, 'Student role not found in system', 500);
            }
    ...
    static async listStudents(req, res, next) {
        try {
            ...
            const isStaffOrAdmin = userRoles.some((r) => ['superadmin', 'admin', 'teacher', 'staff'].includes(r));
            if (!isStaffOrAdmin) {
                return (0, api_response_1.sendError)(res, 'Access denied: Students are not permitted to view other students or student directory', 403);
            }
    ...
    static async getStudent(req, res, next) {
        try {
            const { id } = req.params;
            if (id === 'me') {
                const student = await student_service_1.StudentService.getStudentByUserId(req.user.id);
                if (!student) {
                    return (0, api_response_1.sendError)(res, 'Student profile not found for current user', 404);
                }
                return (0, api_response_1.sendSuccess)(res, student, 'Student profile retrieved successfully', 200);
            }
            const hasAccess = await canAccessStudentRecord(req.user, id);
            if (!hasAccess) {
                return (0, api_response_1.sendError)(res, 'Access denied: Students can only view their own profile and details', 403);
            }
    ...
    static async getEnrollments(req, res, next) {
        try {
            let { id } = req.params;
            if (id === 'me') {
                const student = await client_1.prisma.student.findFirst({ where: { userId: req.user.id, deletedAt: null }, select: { id: true } });
                if (!student)
                    return (0, api_response_1.sendError)(res, 'Student profile not found', 404);
                id = student.id;
            }
            else {
                const hasAccess = await canAccessStudentRecord(req.user, id);
                if (!hasAccess) {
                    return (0, api_response_1.sendError)(res, 'Access denied: You cannot view other students enrollments', 403);
                }
            }
    ...
    static async getAttendance(req, res, next) {
        try {
            ...
            if (id === 'me') {
                ...
                const hasAccess = await canAccessStudentRecord(req.user, id);
                if (!hasAccess) {
                    return (0, api_response_1.sendError)(res, 'Access denied: You cannot view other students attendance', 403);
                }
            }
